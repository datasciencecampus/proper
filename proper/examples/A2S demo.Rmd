---
title: "A2S Example: ATM accessibility for user specified LSOA in the UK"
output:
...
atms_overpass <- opq(paste0(lsoa_name," UK")) %>% # Area bounding box
...
```{r message=FALSE, warning=FALSE}
atms <- locationValidatorIsochrone(output.dir = out_dir,
                                       otpcon = otpcon, locationPoints = atms)
...
leaflet::leaflet(isoc_union) %>%
  ...
  leaflet::addCircleMarkers(lat = atms$lat, 
                            lng = atms$lon,
                            color = 'red',
                            stroke = T, 
                            fill=F,
